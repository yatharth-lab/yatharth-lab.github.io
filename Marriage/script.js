...
async function shareCard(id){
    ...
    ctx.shadowBlur=30;
    ctx.beginPath();
    ctx.roundRect(50,350,980,1420,50);
    ctx.fill();
    ...
    ctx.fillText('💑 GAYATRI VIVAH SUTRA',540,100);

    // Profile Image - Perfect 9:16 Crop (432×768)
    if(p.image){
        const img=new Image();
        img.src=p.image;
        ...
        // Center the 9:16 image
        const imgX=(1080-targetW)/2;
        const imgY=200;
        ctx.drawImage(img,sx,sy,sw,sh,imgX,imgY,targetW,targetH);
    }else{
        const imgX=(1080-432)/2;
        const imgY=200;
        ctx.fillStyle='#FFC107';
        ctx.fillRect(imgX,imgY,432,768);
        ctx.fillStyle='#FFF';
        ctx.font='80px Arial';
        ctx.textAlign='center';
        ctx.fillText(p.gender==='Bride'?'👰':'🤵',540,600);
    }
    ...
    ctx.textAlign='center';
    ctx.fillText(`${p.name}, ${p.age} yrs`,540,1040);

    // Gender
    ctx.fillStyle='#FF8C00';
    ctx.font='35px Arial';
    ctx.fillText(p.gender==='Bride'?'👰 Bride':'🤵 Groom',540,1100);
    ...
    [
        ...
        if(v && v!=='Not specified'){
            ctx.fillStyle='#FFF3E0';
            ctx.beginPath();
            ctx.roundRect(120,y-25,840,55,15);
            ctx.fill();

            ctx.fillStyle='#333';
            ctx.font='bold 30px Arial';
            ctx.textAlign='left';
            ctx.fillText(l+':',140,y+12);

            ctx.fillStyle='#555';
            ctx.font='30px Arial';
            ctx.fillText(v,500,y+12);
    ...
    ctx.textAlign='center';
    ctx.fillText('💑 Gayatri Vivah Sutra',540,1820);
